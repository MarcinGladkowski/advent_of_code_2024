...
def test_determine_operators_combination_for_2_operators():
    """
...
def test_determine_operators_combination_for_3_operators():
    """ Permutations with repeats 3**3 = 27 """
    assert len(determine_operators_combinations(['*', '+', "||"], 3)) == 27

def test_execute_combination():
    assert execute_combination('+*+', [11,6,16,20]) == 292
    assert execute_combination('*||*', [6,8,6,15]) == 7290
...
def test_generate_result_for_test_set_with_concat_operator():
    ...
    assert calculate(test_data, ['*', '+', '||']) == 11387